-- Zero-build repair: Stale Heartbeat Completion Reconciliation Compatibility
-- This migration reuses the Build 0086 function bodies below and changes only
-- heartbeat-evidence selection. Canonical stale heartbeat requests are recorded
-- by the timer as idle_detected with evidence.requested_operation = heartbeat.
-- No historical timer event is rewritten, inserted, deleted, or fabricated.
-- Build 0086 repair: Post-Correction Event-Chain Reconciliation
-- Scope:
--   * Fix the existing Build 0086 completion verifier and transactional
--     reconciliation function so a valid audited correction may be followed by
--     verified resume, heartbeat, activity, and stop events.
--   * Preserve the correction event and every subsequent append-only event.
--   * Require the complete post-correction event chain to reconcile exactly to
--     the final stopped timer-session totals.
--   * Keep existing function signatures, service-role boundary, and workflow.
-- Security:
--   * Service-role only.
--   * No direct browser writes and no parallel timer or completion system.
...
begin
  if to_regprocedure(
    ...
    or to_regprocedure(
      'public.athena_reconcile_feature_completion(uuid,uuid,uuid,text,text,text,jsonb)'
    ) is null
    or to_regclass(
      'public.athena_build_timer_sessions'
    ) is null
    or to_regclass(
      'public.athena_build_timer_events'
    ) is null
  then
    raise exception
      'Build 0086 completion-reconciliation dependencies are missing.';
  end if;
end;
$preflight$;
...
comment on function
  public.athena_read_feature_completion_reconciliation(uuid)
is
  'Build 0086 service-role read-after-write verifier. Validates exact lifecycle, timer, correction-chain, packet, QA, event, build-log, and preparation-package reconciliation.';
...
is
  'Build 0086 transactional completion reconciliation. Accepts a valid audited correction followed by an exact append-only active-time event chain ending at the stopped timer totals.';
