...
export async function updateAthenaProject(formData: FormData) {
  ...
  const updatePayload = {
    ...
    category: readFormValue(formData, "category") || "software",
    project_type: readFormValue(formData, "project_type") || "product",
    definition: readFormValue(formData, "definition"),
    function_summary: readFormValue(formData, "function_summary"),
    build_summary: readFormValue(formData, "build_summary"),
  ...
  await supabase
    .from("athena_products")
    .update({
      name: updatePayload.name,
      description: updatePayload.definition,
